...
let compose_pair (p:(('b -> 'c) * ('a -> 'b))) : 'a -> 'c = fun x ->
  fst p @@ snd p x
...
let rec insert (x:'a) (l:'a list) : 'a list =
  match l with
    | [] -> [x]
    | y::ys -> if x=y then l else if x<y then x::l else y :: insert x ys
...
type poly = (string list * int64) list

let term2exp : string list * int64 -> exp = function
  | ([],c) -> Const c
  | (v::vs,c) -> let e = List.fold_left (fun x y->Mult(x,y)) (Var v) @@ List.map (fun x->Var x) vs in
    if c=1L then e
    else if c=(-1L) then Neg e
    else Mult (Const c, e)

let rec optpoly : poly -> poly = function
  | (v1,c1)::(v2,c2)::r ->
    if v1=v2 then optpoly ((v1, Int64.add c1 c2)::r)
    else (v1,c1) :: optpoly ((v2,c2)::r)
  | xs -> xs

let rec topoly : exp -> poly = function
  | Add (e1, e2) -> topoly e1 @ topoly e2
  | Mult (e1, e2) -> List.concat_map (fun (xs,c1) ->
      map (fun (ys,c2) -> (xs@ys, Int64.mul c1 c2)) (topoly e2)
    ) (topoly e1)
  | Neg e -> map (fun (xs,c) -> (xs, Int64.neg c)) (topoly e)
  | Var s -> [([s],1L)]
  | Const c -> [([], c)]

let optimize (exp:exp) : exp =
(* let optimize exp = *)
  (function
    | e::es -> List.fold_left (fun x y->Add(x,y)) e es
    | _ -> Const 0L)
  @@ map term2exp
  @@ List.filter (fun (_,c)-> not (Int64.equal c 0L))
  @@ optpoly
  @@ List.sort compare
  @@ map (fun (xs,c)-> (List.sort compare xs,c))
  @@ topoly exp
