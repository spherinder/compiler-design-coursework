...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    ...
    let solve (g:Graph.t) : Graph.t =
      ...
      let g_ref = ref g in 
      let w = ref ((Graph.nodes !g_ref) |> Graph.NodeS.to_seq |> Stack.of_seq) in
      
      let _ = while Bool.not @@ Stack.is_empty !w do
        
        let _ = if !debug_solver then print_endline @@ Int.to_string @@ Stack.length !w else () in
        
        let n = Stack.pop !w in

        let out_old = Graph.out !g_ref n in

        let preds = Graph.preds !g_ref n in

        let facts_of_pred = List.map (Graph.out !g_ref) @@ Graph.NodeS.elements preds in

        let out_new = Graph.flow !g_ref n @@ Fact.combine facts_of_pred in

        let _ = g_ref := Graph.add_fact n out_new  !g_ref in 
        
        if Fact.compare out_new out_old  <> 0
        then Graph.NodeS.iter (fun node -> Stack.push node !w)  (Graph.succs !g_ref n) 
        else ()
