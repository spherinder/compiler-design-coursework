...
(*Strategy: graph coloring, no optimistic coloring, no same color preference, use 2 special regs
   for loading spilled operands, save callee save registers when function code long-ish*)
type graph = {nodes:UidS.t; edges: uid -> UidS.t} 

module CGraph: 
sig
  val from_fdecl:Ll.fdecl -> garph 
  val get_neighbors: t -> uid -> UidS.t
  val get_nodes: t -> UidS.t
end = 
struct
    let from_fdecl_liveness fdecl liveness = 
      let f_param set zipped_param = UidS.add (fst zipped_param) set 
      and f_lbl set _ = set
      and f_insn set (u, i) -> UidS.add u set 
      and f_term set (u, t) -> set 
      and init = UidS.empty in
        let nodes = fold_fdecl f_param f_lbl f_insn f_term init
        and edges = liveness.live_in  
    in
      {nodes = nodes; edges = edges}
      
    let get_neighbors g n = g.edges n
    let get_nodes g = g.nodes 
end

let color_graph (g: graph) (k:int) (colors:int UidM.t): int UidM.t =
  
  let nodes_less_k = UidS.filter (fun node -> UidS.cardinal (Graph.get_neighbors g node) < k) in

  if UidS.cardinal nodes_less_k = 0 
  then 
    let to_remove = UidS.choose nodes_less_k in
    let nodes' = UidS.remove to_remove g.nodes  in
    let edges' = (*TODO*) 
    color_graph g' k colors  
  else 
...
let compile_fdecl tdecls (g:gid) (f:Ll.fdecl) : x86stream =
  ...
  (* 
     ...
     respect to liveness.
  *)
    (*TODO: adjust code to save all callee-save registers if > 5 spill-bytes*)
